...
if (isset($_POST['submit'])) {
    if (isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['nom']) && !empty($_POST['nom']) && isset($_POST['prenom']) && !empty($_POST['prenom']) && estInscrit($_POST['email'], $_POST['nom'], $_POST['prenom'])[0] == 0) {
        ...
        header("Location: connexion-valid.php");
        exit;
    } else {
            ...
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" method="POST">
                        ...
                        ?>
                        <button type="submit" id="submit" name="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center focus:ring-4 focus:outline-none focus:ring-blue-300">Se connecter</button>
                    </form>
                </div>
            </div>
        </div>
