    ...
    <div class="relative">
        <div class="relative w-full overflow-x-auto px-20">
            <ul role="list" class="inline-flex mx-0 grid sm:grid-cols-1 space-x-0 md:grid-cols-2 md:gap-x-10 lg:grid-cols-3 lg:gap-x-10">
                <?php
                foreach ($allrattrapages as $cours) {
                ?>
                    <li class="inline-flex flex-col text-center w-auto p-5">
                        <div class="max-w-sm p-6 bg-blue-100 border rounded-lg ">
                            ...
                            <a href="/rattrapage.php?token=<?= $_GET["token"] ?>&id_rattrapage=<?= $cours->getIdCours() ?>" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                                Choisir ce rattrapage
