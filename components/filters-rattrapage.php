<div class="2xl:container 2xl:mx-auto">
    ...
    <div class="md:py-6 lg:px-20 md:px-6 py-5 px-4">
        <div class="flex justify-between items-center">
            <h2 class="lg:text-4xl text-3xl lg:leading-9 leading-7 text-gray-800 font-semibold">Vous avez <?= $utilisateur->getIdUtilsateur() ?> cours à rattraper <?php if ($vendre) { ?> à vendre <?php } ?></h2>
        ...
        <button onclick="showFilters()" class="cursor-pointer mt-6 block sm:hidden py-2 w-full bg-pink-500 flex text-base leading-4 font-normal text-white justify-center items-center">
            <svg class="mr-2" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M6 12C7.10457 12 8 11.1046 8 10C8 8.89543 7.10457 8 6 8C4.89543 8 4 8.89543 4 10C4 11.1046 4.89543 12 6 12Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M6 4V8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                <path d="M6 12V20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
        ...
        <form action="creations.php">
            ...
            <div>
                ...
                <div class="flex flex-wrap mt-4 max-w-lg">
                    <?php
                    foreach ($alljour as $jour) {
                    ?>
                        <div class="flex md:justify-center md:items-center items-center justify-start p-2 pr-6">
                            <input class="w-4 h-4 mr-2 text-pink-500 focus:ring-0" type="checkbox" value="true" id="<?= $jour ?>" name="<?= $jour ?>" <?php if (in_array($jour, $filtres["jour"])) { ?>checked<?php } ?> />
                            <div class="inline-block">
                                <div class="flex space-x-6 justify-center items-center">
                                    <label class="mr-2 text-sm leading-3 font-normal text-gray-600" for="<?= $jour ?>"><?= $jour ?></label>
                                </div>
                            </div>
                        </div>
                    <?php
            ...
            <div>
                ...
                <div class="flex flex-wrap mt-4 max-w-lg">
                    ...
                    ?>
                        <div class="flex md:justify-center md:items-center items-center justify-start p-2 pr-6">
                            <input class="w-4 h-4 mr-2 text-pink-500 focus:ring-0" type="checkbox" id="<?= $horaire ?>" name="<?= $horaire ?>" <?php if (in_array($horaire, $filtres["horaire"])) { ?>checked<?php } ?> />
                            <div class="inline-block">
                                <div class="flex space-x-6 justify-center items-center">
                                    <label class="text-sm leading-3 font-normal text-gray-600" for="<?= $horaire ?>"><?= $horaire ?></label>
                                </div>
                            </div>
                        </div>
                    <?php
